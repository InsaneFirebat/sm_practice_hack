...
update_sprite_oob:
{
  ...
  .loop_x
    ...
    LDA !LEVEL_DATA+1,X : AND #$00F0 : CMP #$00D0 : BEQ .vertical
  ...
  .get_block_gfx
    ...
    ; Get X coord
    LDA $C1 : INC #2 : ASL #4 : SEC : SBC $C5 : STA !OAM_LOW,Y

    ; Get Y coord
    LDA $C3 : CLC : ADC #$04 : ASL #4 : SEC : SBC $C7 : STA !OAM_LOW+$1,Y
  ...
  .check_scroll
    TXA : LSR : TAX
    LDA !LEVEL_BTS,X : AND #$00FF : CMP #$0046 : BEQ .next
    LDX #$0006
    BRA .get_block_gfx
...
block_gfx:
    ...
    ;  air,   slope, air (trick), treadmill, ??,    h-extend, ??,    ??,    solid, door,  spike, crumble, shot,  v-xtend, grapple, bomb
...
custom_sprite_hitbox:
{
    ...
    CMP.w #ROOM_Kraid : BEQ .kraid_bridge
